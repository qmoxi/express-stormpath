...
module.exports = function organizationResolutionGuard(req, res, formActionUri, next) {
  ...
  if (isRootDomainMultiTenantRequest(req)) {
    ...
    return forms.organizationSelectForm.handle(req, {
      success: function (form) {
        ...
        stormpathClient.getOrganizations({nameKey: form.data.organizationNameKey}, function (err, collection) {
          ...
          if (collection.items.length !== 1) {
            ...
            // Create directory and organization with same name.
            // Map directory to organization, and then map organization to application.
            // This is for multi tenant sub domain models with one directory per organization.
            stormpathClient.createDirectory({name: form.data.organizationNameKey}, function (err, directory) {
              if (err) {
                return res.json(err);
              }

              stormpathClient.createOrganization({name: form.data.organizationNameKey, nameKey: form.data.organizationNameKey}, function (err, organization) {
                if (err) {
                  return res.json(err);
                }

                organization.createAccountStoreMapping({accountStore: directory, isDefaultAccountStore: true, isDefaultGroupStore: true}, function (err, organizationAccountStoreMapping) {
                  if (err) {
                    return res.json(err);
                  }

                  application.createAccountStoreMapping({accountStore: organization}, function (err, applicationAccountStoreMapping) {
                    if (err) {
                      return res.json(err);
                    }
            ...
            // If organization already exists possibly do something different here?
      ...
      error: function (form) {
        render(req, res, config.web.organizationSelect.view, {
          form: form,
          formActionUri: formActionUri,
          formModel: organizationSelectFormModel,
